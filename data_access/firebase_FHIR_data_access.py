...
class FirebaseFHIRAccess:
    ...
    def fetch_data(
        ...
        for user in users:
            query = self.db.collection(collection_name).document(user.id).collection(subcollection_name)
            if loinc_codes:
                for code in loinc_codes:
                    display_str, code_str, system_str = self.get_code_details(code)            

                    FHIR_docs = query.where(
                        filter=FieldFilter("code.coding", "array_contains", {
                            'display': display_str,
                            'system': system_str,
                            'code': code_str
                        })
                    ...
                    for doc in FHIR_docs:
                        ...
                        enhanced_fhir_obj = EnhancedObservation(observation=fhir_obj, UserId=user.id)
    ...
    def get_code_details(self, code: str) ->  Tuple[Optional[str], Optional[str], Optional[str]]:
        code_mappings = {
            '9052-2': (
