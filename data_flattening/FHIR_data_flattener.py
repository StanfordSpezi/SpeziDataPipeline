...
def flatten_FHIR_resources(FHIR_resources: List[EnhancedObservation]) -> FHIRDataFrame:
    ...
    for FHIR_obj in FHIR_resources:
        effective_datetime = FHIR_obj.observation.dict()['effectivePeriod']['start'] or FHIR_obj.dict()['effectiveDateTime']
        ...
        quantity_name = coding[1]['display'] if len(coding) > 1 else (coding[0]['display'] if len(coding) > 0 else '')
    ...
    flattened_df['EffectiveDateTime'] = pd.to_datetime(flattened_df['EffectiveDateTime'], errors='coerce')    
